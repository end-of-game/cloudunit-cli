...
public class AdminUtils {
    ...
    public String listUsers() {
        ...
        try {
            MessageConverter.buildListUsers(JsonConverter.getUsers(restUtils
                    .sendGetCommand(
                            authentificationUtils.finalHost + "/admin/users",
    ...
    public String createUser(String login, String firstName, String lastName,
        ...
        try {
            ...
            restUtils
                    .sendPostCommand(authentificationUtils.finalHost
                                    + "/admin/user", authentificationUtils.getMap(),
                            parameters);
            statusCommand.setExitStatut(0);
        } catch (ResourceAccessException e) {
            log.log(Level.SEVERE,
    ...
    public String changeRightsUser(String login, String role) {
        ...
        try {
            ...
            restUtils.sendPostCommand(authentificationUtils.finalHost
                            + "/admin/user/rights", authentificationUtils.getMap(),
                    parameters);
            statusCommand.setExitStatut(0);
        } catch (ResourceAccessException e) {
            log.log(Level.SEVERE,
    ...
    public String deleteUser(String login) {
        ...
        try {
            restUtils.sendDeleteCommand(authentificationUtils.finalHost
                    + "/admin/user/" + login, authentificationUtils.getMap());
            statusCommand.setExitStatut(0);
        } catch (ResourceAccessException e) {
            log.log(Level.SEVERE,
                    "The CLI can't etablished connexion with host servers. Please try later or contact an admin");
    ...
    public String getMessages(String login, String rows) {
        ...
        try {
            MessageConverter.buildUserMessages(JsonConverter
                    .getMessage(restUtils.sendGetCommand(
                            authentificationUtils.finalHost
                                    + "/admin/messages/rows/" + rows
                                    + "/login/" + login,
                            authentificationUtils.getMap()).get("body")));
